...
FLAG = "FLAG{g1t_r3p0_l34k_1s_b4d}"

GIT_HEAD = "ref: refs/heads/main\n"
GIT_LOG_HEAD = """0000000000000000000000000000000000000000 a1b2c3d4e5f6789012345678901234567890abcd Admin <admin@ctf-arena.com> 1692500000 +0000 commit (initial): Initial commit
a1b2c3d4e5f6789012345678901234567890abcd b2c3d4e5f6789012345678901234567890abcdef Admin <admin@ctf-arena.com> 1692500500 +0000 commit: Add database credentials
b2c3d4e5f6789012345678901234567890abcdef c3d4e5f6789012345678901234567890abcdef01 Admin <admin@ctf-arena.com> 1692501000 +0000 commit: Remove hardcoded secret FLAG{g1t_r3p0_l34k_1s_b4d} from config
c3d4e5f6789012345678901234567890abcdef01 d4e5f6789012345678901234567890abcdef0123 Admin <admin@ctf-arena.com> 1692501500 +0000 commit: Update README
"""
...
<head>
    ...
    <title>CTFdpk Arena - Topic 2: Level 4</title>
    <style>
    ...
    <div class="container">
        <div class="challenge-card">
            <div class="challenge-header">
                <div><h2 class="challenge-title">Topic 2: Information Disclosure - Level 4</h2><div class="challenge-meta">Category: Version Control Leak | Difficulty: Hard</div></div>
            </div>
            <div class="description">
                <p>Excellent work finding the .env file, agent. Now we move to a critical misconfiguration often seen in real-world CTFs.</p>
                <p>Developers use Version Control Systems (like Git) to track code changes. Sometimes, they accidentally deploy the version control folder itself to the public web server.</p>
                <p>The flag was once hardcoded in the application, but the developer "removed" it in a later commit. However, history never forgets.</p>
                <p><strong>Your Objective:</strong> Find the exposed version control data and recover the deleted flag from the history. Format: <code>FLAG{...}</code></p>
            </div>
            <div class="hints-section">
                <h3 style="color: var(--accent); margin-bottom: 1rem;">Need Help? (Hints)</h3>
                <details><summary>Hint 1</summary><div class="hint-content">The most popular version control system creates a hidden folder at the root of the project. It starts with a dot.</div></details>
                <details><summary>Hint 2</summary><div class="hint-content">Try accessing <code>/.git/HEAD</code> to confirm the leak exists. Then look for a log file inside <code>/.git/logs/</code> that records commit history.</div></details>
            </div>
...
"""
        ...
        elif parsed_path.path == '/.git/HEAD':
            self.send_response(200)
            self.send_header('Content-type', 'text/plain')
            self.end_headers()
            self.wfile.write(GIT_HEAD.encode())
            
        elif parsed_path.path == '/.git/logs/HEAD':
            self.send_response(200)
            self.send_header('Content-type', 'text/plain')
            self.end_headers()
            self.wfile.write(GIT_LOG_HEAD.encode())
        ...
        if self.path == '/submit':
            ...
            try:
                ...
                if submitted_flag == FLAG:
                    response = {"success": True, "message": "Level 4 Cleared! You recovered the deleted secret."}
                else:
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    print("CTFdpk Arena - Topic 2, Level 4 Started!")
