    ...
    def do_GET(self):
        ...
        if parsed_path.path == '/':
            ...
            if 'auth_token' in cookie:
                try:
                    decoded_bytes = base64.b64decode(cookie['auth_token'].value)
            ...
            if 'auth_token' not in cookie:
                default_data = json.dumps({"user": "netrunner_01", "role": "guest"})
                encoded_data = base64.b64encode(default_data.encode('utf-8')).decode('utf-8')
                self.send_header('Set-Cookie', f'auth_token={encoded_data}; Path=/')
            self.end_headers()
...
<head>
    <meta charset="UTF-8">
    <title>CTFdpk Arena - Topic 3: Level 2</title>
    <style>
        ...
        .dashboard {{ position: relative; z-index: 1; background: rgba(10, 10, 20, 0.95); border: 2px solid #00ffff; box-shadow: 0 0 20px #00ffff, inset 0 0 20px rgba(0, 255, 255, 0.2); padding: 30px; width: 800px; max-width: 90%; border-radius: 5px; }}
        .header {{ border-bottom: 2px solid #ff00ff; padding-bottom: 15px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center; }}
        .header h1 {{ margin: 0; color: #00ffff; text-shadow: 0 0 10px #00ffff; font-size: 1.8rem; }}
        .status-badge {{ background: #ff00ff; color: #000; padding: 5px 15px; font-weight: bold; border-radius: 3px; box-shadow: 0 0 10px #ff00ff; }}
        ...
        .flag-container {{ background: rgba(0, 255, 255, 0.1); border: 1px dashed #00ffff; padding: 20px; margin: 20px 0; text-align: center; display: {'block' if is_admin else 'none'}; }}
        .flag-text {{ font-size: 1.5rem; color: #ffff00; text-shadow: 0 0 10px #ffff00; margin: 10px 0; }}
        .input-group {{ display: flex; gap: 10px; margin-top: 20px; }}
        input[type="text"] {{ flex: 1; background: #000; border: 1px solid #00ffff; color: #00ffff; padding: 10px; font-family: 'Share Tech Mono', monospace; font-size: 1.1rem; outline: none; }}
    ...
    <div class="dashboard">
        <div class="header">
            ...
            <div class="status-badge {'admin' if is_admin else ''}">
                {'ACCESS GRANTED' if is_admin else 'ENCRYPTED'}
            </div>
        </div>

        <div class="content">
            <p>> SYSTEM STATUS: <span class="highlight">SECURE CHANNEL ACTIVE</span></p>
            <p>> AUTHENTICATION TOKEN: <span class="highlight">DETECTED</span></p>
            <p>> CLEARANCE LEVEL: <span class="{'highlight' if is_admin else 'danger'}">{'OMEGA' if is_admin else 'RESTRICTED'}</span></p>
            
            {'<p>> ACCESSING CLASSIFIED DATABASE...</p>' if is_admin else '<p class="danger">> ERROR: TOKEN DECODED BUT INSUFFICIENT PRIVILEGES. FLAG ENCRYPTED.</p>'}
        </div>
        ...
        <details>
            ...
            <div class="hint-text">
                > HINT 1: The server no longer stores plaintext roles. It uses a standard encoding scheme often mistaken for encryption. Look at the raw cookie value in your browser storage.<br>
                > HINT 2: Extract the raw cookie string, decode it using your terminal tools, modify the internal payload, re-encode it, and replace the cookie in your browser.
            </div>
        </details>
    </div>
    ...
    <script>
        ...
        document.getElementById('submit-btn').addEventListener('click', async () => {{
            ...
            try {{
                ...
                if (data.success) {{ alert('[+] TRANSMISSION SUCCESSFUL. FLAG VERIFIED.'); }} 
                else {{ alert('[!] TRANSMISSION FAILED: ' + data.message); }}
        ...
        if self.path == '/submit':
            ...
            try:
                ...
                if submitted_flag == FLAG:
                    response = {"success": True, "message": "Level 2 Cleared!"}
                else:
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    print("CTFdpk Arena - Topic 3, Level 2 (Blind Base64) Started!")
